...
- **未授权访问自动对比**：对浏览器带认证抓到的真实 API 做剔除 Cookie 的无认证重放并比对两侧响应（完全一致 → JSON 结构/字段值 → 鉴权失败标记排除），命中直接输出未授权访问证据到 `9-未授权访问检测结果.txt`；浏览器捕获的真实 API 也会并入无参探测队列。需配合 `-c` 携带登录态使用，详见 [docs/未授权访问自动对比设计.md](docs/未授权访问自动对比设计.md)
